	...
	root := r.Group("/", rootMw()...)
	{
		_douyin := root.Group("/douyin", _douyinMw()...)
		{
			_publish := _douyin.Group("/publish", _publishMw()...)
			{
				...
				_action.POST("/", append(_publish_ctionMw(), api_user.PublishAction)...)
			}
			{
				_list := _publish.Group("/list", _listMw()...)
				_list.GET("/", append(_publishlistMw(), api_user.PublishList)...)
			}
		}
		{
			_user := _douyin.Group("/user", _userMw()...)
			_user.GET("/", append(_userinfoMw(), api_user.UserInfo)...)
			{
				_login := _user.Group("/login", _loginMw()...)
				_login.POST("/", append(_loginuserMw(), api_user.LoginUser)...)
			}
			{
				_register := _user.Group("/register", _registerMw()...)
				_register.POST("/", append(_registeruserMw(), api_user.RegisterUser)...)
			}
		}
	}
}
